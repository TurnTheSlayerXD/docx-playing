"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.StyleForCharacter = void 0;
var properties_1 = require("@file/paragraph/run/properties");
var style_1 = require("./style");
var StyleForCharacter = /** @class */ (function (_super) {
    __extends(StyleForCharacter, _super);
    function StyleForCharacter(options) {
        var _this = _super.call(this, { type: "character", styleId: options.id }, __assign({ uiPriority: 99, unhideWhenUsed: true }, options)) || this;
        _this.runProperties = new properties_1.RunProperties(options.run);
        _this.root.push(_this.runProperties);
        return _this;
    }
    return StyleForCharacter;
}(style_1.Style));
exports.StyleForCharacter = StyleForCharacter;
